...
tests/    22 pytest tests -- run `pytest` after changing rules.yaml
```
Testing the rules with `test_rules.py` to check the decisions on constructed header rows, `test_pipeline.py` runs the whole chain over the eight files in `demo/sample_data` (images of MONOCHROME1, two-hand, wrist, landscape single hand). 
